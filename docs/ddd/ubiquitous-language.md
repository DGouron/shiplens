...
| Manual cycle themes refresh | Analytics | Client-side action triggered by the refresh button on the Top 5 cycle themes widget — increments a `forceRefreshToken` in `useTopCycleThemes`, which both removes the existing React Query cache entry AND causes the next HTTP call to carry `?refresh=true`, invalidating the backend `CycleThemeSet` cache for the active cycle before re-invoking the AI |
| Top cycle themes | Analytics | Ranking of AI-inferred semantic clusters (themes) of the selected team's active cycle by chosen metric (count / points / time) — third widget of the dashboard right-side column, below top projects and top assignees |
| Theme ranking row | Analytics | One row of the Top 5 cycle themes widget, showing `themeName` + `metricValueLabel` — clicking it opens the cycle theme issues drawer for that theme |
| Cycle theme issues drawer | Analytics | Right-side panel listing the issues classified by the AI under a clicked theme — `externalId`, `title`, `assigneeLabel`, `pointsLabel`, `statusName`, optional Linear link. Does NOT render `cycleTimeLabel` (distinct from the assignee drawer) |
| AI unavailable card | Analytics | UX decision: when the cycle themes endpoint returns `ai_unavailable`, the widget stays visible with an amber warning tone (`emptyTone: 'warning'`) and an active refresh button — overrides the original spec's "card hidden" rule to let the user retry without leaving the dashboard |
| Force refresh token | Analytics | Monotonic integer state inside `useTopCycleThemes` used as part of the React Query key — bumping it forces a refetch AND flips the HTTP `refresh` query param to `true`. Reset to 0 on team switch so a new team does not inherit a stale refresh intent |
