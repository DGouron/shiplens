...
import { MissingPackmindTokenError } from '@modules/audit/entities/packmind/packmind.errors.js';
import { NoPracticesFoundError } from '@modules/audit/entities/packmind/packmind.errors.js';
import { PackmindUnreachableWithoutCacheError } from '@modules/audit/entities/packmind/packmind.errors.js';
import { InvalidPackmindTokenError } from '@modules/audit/entities/packmind/packmind.errors.js';
...
describe('SyncPackmindRulesUsecase', () => {
  ...
  describe('resynchronization', () => {
    it('updates existing rules and adds new ones', async () => {
      const existingRule = new AuditRuleBuilder().withIdentifier('PM-1').withName('Old name').withOrigin('packmind').build();
      await auditRuleGateway.save(existingRule);

      const practices: PackmindPractice[] = [
        { identifier: 'PM-1', name: 'Updated name', measurable: true, conditionExpression: 'cycle time > 5 jours', severity: 'warning' },
        { identifier: 'PM-2', name: 'New rule', measurable: true, conditionExpression: 'lead time > 10 jours', severity: 'error' },
      ];
      const packmindGateway = new StubPackmindGateway(practices);
      const usecase = new SyncPackmindRulesUsecase(auditRuleGateway, checklistItemGateway, packmindGateway);
  ...
  describe('Packmind unreachable', () => {
    it('returns cached rules with warning when cache exists', async () => {
      const cachedRule = new AuditRuleBuilder().withIdentifier('PM-1').withOrigin('packmind').build();
    ...
    it('rejects when unreachable and no cache', async () => {
      ...
      await expect(usecase.execute({ token: 'valid-token' })).rejects.toThrow(PackmindUnreachableWithoutCacheError);
    });
  });
  ...
  describe('token validation', () => {
    it('rejects invalid token', async () => {
      ...
      await expect(usecase.execute({ token: 'invalid-token' })).rejects.toThrow(InvalidPackmindTokenError);
    });
    ...
    it('rejects missing token', async () => {
      ...
      await expect(usecase.execute({ token: '' })).rejects.toThrow(MissingPackmindTokenError);
    });
  });
  ...
  describe('no practices available', () => {
    it('rejects when Packmind has no practices', async () => {
      ...
      await expect(usecase.execute({ token: 'valid-token' })).rejects.toThrow(NoPracticesFoundError);
    });
  });
});
