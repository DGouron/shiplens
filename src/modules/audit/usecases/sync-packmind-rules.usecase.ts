...
@Injectable()
export class SyncPackmindRulesUsecase
  implements Usecase<SyncPackmindRulesParams, SyncResult>
{
  constructor(
  ...
  private async handleUnreachable(): Promise<SyncResult> {
    ...
    const packmindItems = cachedItems.filter(
      (item) => item.origin === 'packmind',
    );
  ...
  private async syncPractices(
    practices: PackmindPractice[],
  ): Promise<SyncResult> {
