...
@Injectable()
export class EvaluateAuditRuleUsecase
  implements Usecase<EvaluateAuditRuleParams, EvaluationResult>
{
  ...
  async execute(params: EvaluateAuditRuleParams): Promise<EvaluationResult> {
    const rule = await this.auditRuleGateway.findByIdentifier(
      params.identifier,
    );
    if (!rule) {
