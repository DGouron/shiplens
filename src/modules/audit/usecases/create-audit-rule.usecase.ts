...
@Injectable()
export class CreateAuditRuleUsecase implements Usecase<CreateAuditRuleParams, void> {
  ...
  async execute(params: CreateAuditRuleParams): Promise<void> {
    ...
    const existing = await this.auditRuleGateway.findByIdentifier(rule.identifier);
    if (existing) {
